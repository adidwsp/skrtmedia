...
export interface TicketCardProps {
  ticket: Ticket & { // extend with optional common DB fields
    ...
    created_at?: string;
    // any other fields from DB accessible via ticket object
  };
}
...
const TicketCard: React.FC<TicketCardProps> = ({ ticket }) => {
  ...
  // Build confirmation URL (when scanned QR, user will open this page)
  const confirmationUrl = useMemo(() => {
    // window is available because this is client component
    try {
      const origin = typeof window !== "undefined" ? window.location.origin : "";
      // prefer full URL to confirmation page
  ...
  // QR image src using public QR generator (no extra package). Size 300x300
  const qrImageSrc = useMemo(() => {
    const data = encodeURIComponent(confirmationUrl);
    return `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${data}`;
  }, [confirmationUrl]);

  const purchaser = (ticket.purchaserName || ticket.name || ticket.name) ?? null;
    ...
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden max-w-lg mx-auto my-8 border border-gray-100 transition duration-300 hover:shadow-2xl">
      ...
      <TicketModal title="QR Code Tiket" isOpen={showQrModal} onClose={() => setShowQrModal(false)}>
        <div className="flex flex-col items-center space-y-4">
          {/* show QR image (generated by public API) */}
          <img src={qrImageSrc} alt={`QR for ${ticket.id}`} width={300} height={300} className="rounded-md border border-gray-200" />
          <div className="text-sm text-center text-gray-600">
            Pindai kode ini untuk membuka halaman konfirmasi tiket.
          </div>
      ...
      <TicketModal title="Detail Tiket" isOpen={showDetailModal} onClose={() => setShowDetailModal(false)}>
        <div className="space-y-3 text-sm text-gray-700">
          ...
          <div className="pt-2 text-xs text-gray-500">
            Jika ada kesalahan data, hubungi admin untuk perbaikan.
          </div>
        </div>
      </TicketModal>
    </div>
