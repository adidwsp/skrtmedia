...
export default async function TicketConfirmationPage({ params }: Props) {
  ...
  const ticketData = {
    ...
    date: ev?.date ?? ev?.event_date ?? (ticketRow.created_at ? new Date(ticketRow.created_at).toISOString() : ""),
    ...
    purchaserName: ticketRow.name ?? ticketRow.nama ?? "",
    email: ticketRow.email ?? "",
    phone: ticketRow.phone ?? "",
    donation_amount: typeof ticketRow.donation_amount === "number" ? ticketRow.donation_amount : ticketRow.price ?? 0,
    donation_bank: ticketRow.donation_bank ?? "",
        ...
        <div className="bg-white rounded-2xl shadow-md p-6">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center justify-between gap-3 bg-gray-100 px-4 py-3 rounded mt-2">
              <div className="font-mono font-semibold text-lg">{ticketData.id}</div>
              <TicketActionsClient
                ticketId={ticketData.id}
                waUrl={waUrl}
                groupLink={groupLink}
                qrUrl={ticketData.qrCodeUrl}
              />
            </div>
          </div>
